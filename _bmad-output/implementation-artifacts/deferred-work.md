...
- **[MED] Story 12.2 review** NFR-S9 subprocess sandbox hardening (filesystem/network restrictions, memory limits, path-traversal validation) not implemented; scheduled for Story 12.5 -- [full entry](#deferred-from-code-review-of-12-2-subprocess-jsruntime-map-reduce-views-2026-04-17)
- **[MED] Story 12.2 review** `JSRUNTIMETIMEOUT` documented as enforced but `Pipe.cls` does not pass a timeout to `$ZF(-100)`; docs and code disagree -- [full entry](#deferred-from-code-review-of-12-2-subprocess-jsruntime-map-reduce-views-2026-04-17)
...
- **[MED] Story 12.3 review** `ListValidateFunctions` iterates ALL live docs via `ListLiveDocIds(pDB, 1)` then filters on `_design/` prefix; on million-doc DBs this is O(N) per write. Direct `$Order` over `^IRISCouch.Docs(pDB, "_design/...")` would be O(k) in the number of design docs. Performance follow-up; defer with Story 12.5 pool work -- [full entry](#deferred-from-code-review-of-12-3-subprocess-jsruntime-validation-and-filter-functions-2026-04-17)
- **[MED] Story 12.3 review** One subprocess spawn per changes-feed doc for custom filters; filter-heavy feeds multiply the 80-150ms cold-start cost. The `ddoc filters` protocol supports batched `[[doc1,doc2,...], req]` invocation but the handler issues per-doc calls. Deferred to Story 12.5 (persistent pool) per story spec -- [full entry](#deferred-from-code-review-of-12-3-subprocess-jsruntime-validation-and-filter-functions-2026-04-17)
