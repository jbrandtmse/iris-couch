...
- **MatchesPartialFilter only handles top-level equality selectors** [Projection/MangoIndex.cls:244-265] -- Complex selectors ($gt, $in, nested) are not evaluated during index population. Documents not matching a complex partial_filter_selector will still be indexed. Full selector evaluation planned for Story 6.2.
...
- **Missing cross-type comparison unit tests** [MangoSelectorTest.cls] -- Story Task 8 specifies TestCompareNullLtNumber and TestCompareBoolLtString tests but only TestCompareNumberLtString was implemented. These tests would exercise the TypeRank type hint paths and validate CouchDB's null < false < true < numbers < strings ordering. Add when test coverage is expanded or when TypeRank type hints are wired through more call paths.
